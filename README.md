...
`make train` and `make sweep` require an `.env` file located at the root of the project directory that contains a Weighs & Biases API key. To train without one, run locally (see below) with the flag `use_wandb=0`.
